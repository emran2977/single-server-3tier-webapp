...
```bash
cd /home/ubuntu
git clone https://github.com/YOUR_USERNAME/single-server-3tier-webapp.git
...
scp -i your-key.pem -r ./single-server-3tier-webapp ubuntu@YOUR_EC2_PUBLIC_IP:/home/ubuntu/
...
```bash
# On local machine:
zip -r single-server-3tier-webapp.zip single-server-3tier-webapp
scp -i your-key.pem single-server-3tier-webapp.zip ubuntu@YOUR_EC2_PUBLIC_IP:/home/ubuntu/

# On server:
sudo apt install -y unzip
unzip single-server-3tier-webapp.zip
cd single-server-3tier-webapp
```
...
**Note**: When updates include new migrations, you'll run them the same way. See Part 13 for update procedures.
...
## Part 5: Server Structure Overview

After deploying the application, your server structure looks like:
...
│       └── node_modules/
│
/var/www/bmi-tracker/               # Production frontend files
├── index.html
├── assets/
│   ├── index-[hash].js
│   └── index-[hash].css
...
- Navigating between backend/frontend
...
## Part 6: Process Management with PM2
...
## Part 9: Firewall Configuration (UFW)
...
### 10.3 Use Strong Database Password
...
- Backup database daily (see Part 12.3)
- Backup application code to Git
- Consider AWS snapshots for entire EC2 instance

### 10.6 Monitor Security Updates

```bash
# Check for security updates
sudo unattended-upgrade --dry-run

# View security update logs
...
## Part 12: Daily Maintenance & Troubleshooting

### 12.1 Common Maintenance Tasks
...
### 12.4 Server Resource Monitoring
...
## Part 13: Application Updates & Deployment
...
### 13.2 Update Backend (Node.js API)
...
# Apply new migrations (same process as initial deployment)
psql -U bmi_user -d bmidb -h localhost -f migrations/003_new_migration.sql
# (Replace with actual migration file name)
...
### 13.3 Update Frontend (React App)
...
### 13.4 Update Database Schema (New Migrations)
...
psql -U bmi_user -d bmidb -h localhost -f migrations/003_add_new_feature.sql
...
for migration in migrations/*.sql; do
    ...
    psql -U bmi_user -d bmidb -h localhost -f "$migration"
    if [ $? -eq 0 ]; then
...
for migration in migrations/*.sql; do
    if [ -f "$migration" ]; then
        ...
        psql -U bmi_user -d bmidb -h localhost -f "$migration" 2>&1 | grep -v "already exists" || true
    fi
done
...
### 13.7 Update Monitoring & Health Checks
...
### 13.8 Automated Update with GitHub Webhooks (Optional)
...
### 13.9 Scheduled Maintenance Tasks
...
0 2 * * * /home/ubuntu/backup-db.sh >> /var/log/postgresql/backup.log 2>&1
...
### 13.11 Quick Update Commands Reference
...
### 14.4 Additional Cost Optimization Tips
...
find /home/ubuntu/backups -name "bmidb_*.sql" -mtime +7 -delete
...
**Version**: 3.0  
**Changes**: Complete reorganization for logical flow: Server structure after deployment, Testing after Nginx, Security before SSL, Updates separated from daily maintenance
